'use strict';
module.exports = {
	extends: ['core.components.logger.logger', 'core.components.inheritance.api'],
	_interfaces: null,
	...
	_loopThroughFactories: function(factoriesAndInheritance) {
		for (var x in factoriesAndInheritance) {
			if (factoriesAndInheritance[x].implements) {
				this._checkImplementsProperty(x, factoriesAndInheritance[x].implements);
			}
		}
	}, 
	...
	_checkImplementsProperty: function(moduleName, shouldImplement) {
		...
		this.error(
			'Property "implements" of module "%s" should be string or array with interface namespaces', 
			moduleName
		);
	}, 
	...
	_moduleDidImplementInterface: function(moduleName, interfacesNamespace) {
		...
		if (!Array.isArray(iFace)) {
			return this.error(
				'Interface with namespace "%s" should export object with array property named "interface"', 
				interfacesNamespace
			);
		}
	...
	_interfaceMethodIsCorrect: function(iMethod) {
		if (!iMethod || typeof iMethod.method !== 'string' || !Array.isArray(iMethod.arguments)) {
			return false;
		}
	 	return true;
	 },  
