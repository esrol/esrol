...
module.exports = class Esrol {
  ...
  constructor(absPath) {
    ...
    let appPath = absPath.replace(path.join(modulePath, 'index.js'), '');
  ...
  _createApp(json, esrolPath, appPath) {
    if (json.config && json.config.esrol && json.config.esrol.app) {
      return this._dependenciesOnly(json, appPath);
    }
  ...
  _fullInstall(json, esrolPath, appPath) {
    ...
    .then(() => {
      fs.unlink(path.join(appPath, 'esrol-package.json'));
    })
    .then(() => {
      return this._mergeBothPackages(json, esrolJson);
    })
    .then(() => {
      return new Manager(esrolJson.esrolDependencies, appPath);
    })
    .then(() => {
      return this._updateConfig(json);
    })
    .then(() => {
      return this._writePackageJson(json, appPath);
    })
    .then(() => {
      return this._done();
    });
  ...
  _dependenciesOnly(json, appPath) {
    return new Manager(json, appPath);
  }
